...
class FirebaseProduct {
  ...
  Future<List<ProductModel>> getProductListWithLimit(int limit) async {
    try {
      querySnapshot = await firebaseFirestore.collection(Constants().products).orderBy("createdTime", descending: true).limit(limit).get();
  ...
  Future<List<ProductModel>> getProductListByCategory(CategoryModel category, int limit) async {
    try {
      querySnapshot = await firebaseFirestore.collection(Constants().products).where("productCategoryList", arrayContains: category.toJson()).limit(limit).get();
  ...
  Future<List<ProductModel>> getProductListByCategoryList(List<CategoryModel> categories, int limit) async {
    try {
      ...
      QuerySnapshot querySnapshot = await firebaseFirestore.collection(Constants().products).where("productCategoryIdList", arrayContainsAny: categoryIds).limit(limit).get();

      List<ProductModel> productList = querySnapshot.docs.map((doc) => ProductModel.fromJson(doc.data() as Map<String, dynamic>)).toList();
  ...
  Future<List<ProductModel>> getSearchedProduct(String searchText, int limit) async {
    try {
      querySnapshot = await firebaseFirestore.collection(Constants().products).where("productName", isGreaterThan: searchText).limit(limit).get();
  ...
  Future<List<ProductModel>> getProductListBySeller(String sellerId, int limit) async {
    try {
      querySnapshot = await firebaseFirestore.collection(Constants().products).where("sellerId", isEqualTo: sellerId).orderBy("createdTime", descending: true).limit(limit).get();
