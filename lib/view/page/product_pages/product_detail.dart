...
import 'package:handvibe/view/organism/mini_profile_banner.dart';
...
import 'package:handvibe/view/page/profile_pages/profile_page.dart';
...
class ProductDetail extends StatefulWidget {
  ...
  const ProductDetail({super.key, required this.productModel, required this.myId});
    ...
    return Consumer<UserProvider>(builder: (context, userProvider, widgets) {
      return Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: [
            widget.myId == widget.productModel.sellerId
                ? IconButton(
                    onPressed: () async {
                      showDialog(
                        context: context,
                          ...
                          yesMethod: progressing != Progressing.busy
                              ? () async {
                                  if (progressing != Progressing.busy) {
                                    ...
                                    await FirebaseProduct().deleteProduct(widget.productModel.productName);
                                    if (context.mounted) {
                                      Provider.of<ProductProvider>(context, listen: false).getProductsWithLimit(10);
                                      products = userProvider.userProfile!.products;
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            Text(
                              UsefulMethods().dateTimeToTurkishDate(widget.productModel.createdTime, context),
                              style: TextFont().ralewayRegularMethod(16, ColorBank().secondaryText, context),
                            ),
                          ],
                        ),
                        userProvider.userProfile != null
                            ? MiniProfileBanner(
                                profileModel: userProvider.userProfile!,
                                myId: widget.myId,
                                followAction: () async {
                                  if (widget.myId != "") {
                                    setState(() {
                                ...
                                goToProfileAction: () {
                                  if (widget.myId != userProvider.userProfile!.profileId) {
                                    UsefulMethods().navigatorPushMethod(
                                      context,
                                      ProfilePage(profileModel: userProvider.userProfile!, myID: widget.myId),
                                    );
                                  } else {}
                                },
                                follow: follow,
